...
def compute_stats(df):
    ...
    return np.array(N0), np.array(xstd), np.array(ystd)

N0_100, xstd_100, ystd_100 = compute_stats(df100)
N0_200, xstd_200, ystd_200 = compute_stats(df200)
N0_500, xstd_500, ystd_500 = compute_stats(df500)

pixel_size = 25.0
bins = np.arange(100, 1000, 100)

def bin_std(N0, std):
    return np.array([np.std(std[(N0 >= bins[i]) & (N0 < bins[i+1])]) for i in range(len(bins) - 1)])

xstd_binned_100 = bin_std(N0_100, xstd_100)
ystd_binned_100 = bin_std(N0_100, ystd_100)
xstd_binned_200 = bin_std(N0_200, xstd_200)
ystd_binned_200 = bin_std(N0_200, ystd_200)
xstd_binned_500 = bin_std(N0_500, xstd_500)
ystd_binned_500 = bin_std(N0_500, ystd_500)

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8, 4))

# Plot for x-stduracy standard deviations
...
# Plot for y-stduracy standard deviations
