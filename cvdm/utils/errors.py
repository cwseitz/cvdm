...
def errors2d(xy,xy_est):
    ...
    for n in range(num_found):
        ...
        if xy.shape[1] > 0:
            xy,bool,xerr,yerr,label,N0 = match_on_xy(this_xy_est,xy)
            bfound.append(bool)
    ...
    union = nspots+fp
    fn = nspots - inter
    print(nspots,num_found,inter,union,fp,fn)
...
def prepare_data(path,xfiles,yfiles,zfiles):
    ...
    for xfile,yfile,zfile in zip(xfiles,yfiles,zfiles):
        print(xfile,yfile,zfile)
        X = imread(path+'/'+xfile)
        Y = imread(path+'/'+yfile)
        Z = imread(path+'/'+zfile)
        #Z = Z-rolling_ball(Z,radius=20.0)
...
def get_errors(spots,coordsgt,upsample=4):
    ...
    set_metrics = np.array(set_metrics)
